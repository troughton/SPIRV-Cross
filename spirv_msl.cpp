...
void CompilerMSL::emit_custom_functions()
{
	...
	for (auto &spv_func : spv_function_implementations)
	{
		...
		case SPVFuncImplArrayCopy:
			...
			statement("inline void spvArrayCopyFromStack1(thread T (&dst)[N], thread const T (&src)[N])");
			begin_scope();
			...
			statement("template<typename T, uint N>");
			statement("inline void spvArrayCopyFromConstant1(thread T (&dst)[N], constant T (&src)[N])");
			begin_scope();
			statement("for (uint i = 0; i < N; dst[i] = src[i], i++);");
			end_scope();
			statement("");
		...
		case SPVFuncImplArrayOfArrayCopy6Dim:
		{
			...
			for (uint32_t variant = 0; variant < 2; variant++)
			{
				...
				statement("inline void spvArrayCopy", function_name_tags[variant], dimensions, "(thread T (&dst)", array_arg,
